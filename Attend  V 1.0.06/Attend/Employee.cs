...
namespace Attend
{
    public partial class Employee : Form
    {
        ...
        {
            sqlConnection = new SQLiteConnection("Data Source = C:\\Attend  V 1.0.06\\Attend\\bin\\Debug\\Attend.db");
        }
        ...
        private void Grid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                cboG.Text = Grid.SelectedRows[0].Cells[4].Value.ToString();
                txtC.Text = Grid.SelectedRows[0].Cells[5].Value.ToString();
                txtA.Text = Grid.SelectedRows[0].Cells[6].Value.ToString();
                txtPC.Text = Grid.SelectedRows[0].Cells[7].Value.ToString();
                txtMA.Text = Grid.SelectedRows[0].Cells[8].Value.ToString();
                txtPT.Text = Grid.SelectedRows[0].Cells[9].Value.ToString();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Attend", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void btnUP_Click(object sender, EventArgs e)
        {
            ...
            {
                sqlConnection1.ConnectionString = "Data Source =  C:\\Attend  V 1.0.06\\Attend\\bin\\Debug\\Attend.db";
                sqlConnection1.Open();
                using (SQLiteCommand sqlCommand1 = new SQLiteCommand(sqlConnection1))
                {
                    sqlCommand1.CommandText =
                        ("update Employee set First_Name='" +
                        txtFN.Text + "', Last_Name ='" + txtLN.Text +
                        "' ,Gender='" + cboG.Text + "' ,Date ='" + datePicker.Text + "' " +
                        ",Cellphone_Number='" + txtC.Text + "',Address='" + txtA.Text + "'" +
                        " ,Postal_Code='" + txtPC.Text + "',Mail='" + txtMA.Text + 
                        "',Position='" + txtPT.Text + "' where Personal_ID='" + txtPID.Text + "'");
                    ...
                    sqlCommand1.Parameters.Add("Date", DbType.String).Value = datePicker.Text;
